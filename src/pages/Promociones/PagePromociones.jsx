...
import './style/PagePromociones.css'
import { DataContext } from '../../data/Data'
import CardTipo01 from "../../components/Cards/cardTipo01";
import { Slider } from "primereact/slider";
import PopUpCotizacion from "../Catalogo/Components/PopUpCotizacion/PopUpCotizacion";

export default function PagePromociones() {
    ...
    return (
        <div className="PagePromociones w-full p-4 pt-8 flex gap-2 justify-content-end">
            <div className={`Filtros ${ActivarFiltros ? 'ActivoFiltros' : ''}`}>
                ...
                <div className="ContenidoFiltros w-full h-full flex flex-column gap-3 align-items-center justify-content-start pt-4 pb-6">
                    ...
                    <div className="OneFiltro flex flex-column gap-1 p-2">
                        <div className="Titulo">DESCUENTO:</div>
                        <div className="Input Precio">
                            <Slider value={value} onChange={(e) => setValue(e.value)} className="w-10 m-auto" range />
                        </div>
                        <style>
                            {`
                            .PagePromociones .Filtros .ContenidoFiltros .OneFiltro .Input.Precio .p-slider span.p-slider-handle:nth-child(2),
                            .PagePromociones .Filtros .ContenidoFiltros .OneFiltro .Input.Precio .p-slider span.p-slider-handle:nth-child(3){
                            display: flex;
                            align-items:center;
                            justify-content: center;
                            }
                            .PagePromociones .Filtros .ContenidoFiltros .OneFiltro .Input.Precio .p-slider span.p-slider-handle:nth-child(2)::before {
                                content:'${(value[0]).toFixed(1)}%';
                                ...
                                .PagePromociones .Filtros .ContenidoFiltros .OneFiltro .Input.Precio .p-slider span.p-slider-handle:nth-child(3)::before {
                                content:'${(value[1]).toFixed(1)}%';
                                background-color: #116311;
                                font-size: 13px;
                                color: white;
                                padding: 4px;
            ...
            <div className={`Contenido ${ActivarFiltros ? 'ActivoFiltros' : ''} flex flex-wrap w-full pl-5 gap-3`}>
                {Productos?.map((product) => {
                    ...
                    const PrecioMin = parseFloat(Math.min(...value));
                    const PrecioMax = parseFloat(Math.max(...value));
                    const OfertaFil = parseFloat(product.Oferta);

                    const Precio = PrecioMin <= OfertaFil && OfertaFil <= PrecioMax;
                    ...
                    const Oferta = product.Oferta || product?.Oferta > 0 ? true : false;
